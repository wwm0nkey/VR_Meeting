...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] VR_PlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is VR_PlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (VR_PlayerNetworkObject.Length > 0 && VR_PlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(VR_PlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<VR_PlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateVR_Player instead, its shorter and easier to type out ;)")]
		public VR_PlayerBehavior InstantiateVR_PlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(VR_PlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<VR_PlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<VR_PlayerBehavior>().networkObject = (VR_PlayerNetworkObject)obj;
		...
		public VR_PlayerBehavior InstantiateVR_Player(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(VR_PlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<VR_PlayerBehavior>();
